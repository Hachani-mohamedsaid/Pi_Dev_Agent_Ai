...
class _GoalsPageState extends State<GoalsPage> {
  final GoalsApiService _apiService =
      InjectionContainer.instance.goalsApiService;
  List<Goal> _goals = [];
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) {
          return AlertDialog(
            ...
            title: Text(
              'Progression',
              style: TextStyle(color: _primaryText(context), fontSize: 18),
            ),
            content: Column(
              ...
              children: [
                ...
                SliderTheme(
                  ...
                  child: Slider(
                    ...
                    divisions: 20,
                    onChanged: (v) =>
                        setDialogState(() => selectedProgress = v.round()),
                  ),
                ),
                Row(
                  ...
                  children: [0, 25, 50, 75, 100].map((p) {
                    return TextButton(
                      onPressed: () =>
                          setDialogState(() => selectedProgress = p),
                      child: Text(
                        '$p%',
                        style: TextStyle(
                          color: selectedProgress == p
                              ? AppColors.cyan400
                              : _secondaryText(context),
                          fontWeight: selectedProgress == p
                              ? FontWeight.bold
                              : FontWeight.normal,
                        ),
                      ),
  ...
  Future<void> _toggleAction(Goal goal, GoalAction action) async {
    ...
    if (goal.id.startsWith('local_')) {
      final newActions = goal.dailyActions
          .map(
            (a) => a.id == action.id ? a.copyWith(completed: !a.completed) : a,
          )
          .toList();
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                  color: AppColors.cyan500.withOpacity(0.6),
                                  width: 1.5,
                                ),
                              ),
                              child: Text(
                        ...
                        if (_error == null && _goals.isNotEmpty)
                          ..._goals.asMap().entries.map((entry) {
                            ...
                            return Padding(
                              ...
                              child:
                                  _buildGoalCard(context, isMobile, goal, index)
                                      .animate()
                                      .fadeIn(
                                        delay: Duration(
          ...
          children: [
            ...
            Expanded(
              child: Text(
                ...
                style: TextStyle(color: _primaryText(context), fontSize: 13),
              ),
            ),
            TextButton(
              onPressed: _loadData,
